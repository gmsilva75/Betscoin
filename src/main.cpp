...
const string strMessageMagic = "betscoin Signed Message:\n";
...
int64_t GetProofOfStakeReward(int nHeight, int64_t nCoinAge, int64_t nFees)
{
    int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);
            ...
            if(nBestHeight <= 300)
            {
            nSubsidy = 10000 * COIN;  //no pos
            }
			else if(nBestHeight <= 105120)  //2096400 BST
            {
            nSubsidy = 20 * COIN ;  //
            }
			else if(nBestHeight <= 210240)  //1576800 BST
            {
            nSubsidy = 15 * COIN ;  //
            }
			else if(nBestHeight <= 315360)  //1051200 BST
            {
            nSubsidy = 10 * COIN ;  //
            }
			else if(nBestHeight <= 420480)  //525600 BST
            {
            nSubsidy = 5 * COIN ;  //
            }
            else
            {
            nSubsidy = 2.5 * COIN ;  // 262800 BST per year
            }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "betscoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "SuperCoin tipo superman só que coin";
        CTransaction txNew;
        txNew.nTime = 1601737823;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1601737823;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 82550 : 82550;
/*
		...
		*/
        assert(block.hashMerkleRoot == uint256("0x4fb862118d93601265382a27a6ee08453a6ff3cbb29ccd12db60a3b20caa9c6e"));
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // bst: send latest proof-of-work block to allow the
